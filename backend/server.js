  ...
  try {
    ...
    const encuesta = await executeQuerySingle(
      `SELECT IdEncuesta, Nombre,
              ...
              TutorActivo, DocenteActivo, Activa
...
app.post("/api/admin/activar-periodo", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    ...
    await executeQuery(
      `UPDATE dbo.PeriodoEscolar SET Situación = 1 WHERE IdPerio = ?`,
      [idPeriodo]
    )
...
app.post("/api/admin/configurar-evaluacion", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    ...
    const convertirFecha = (f) => {
      if (!f) return null
      if (f.match(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/)) return f
      return f.replace("T", " ") + ":00"
    }
...
app.get("/api/alumno/perfil", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const encuesta = await executeQuerySingle(
      `SELECT IdEncuesta, FechaInicioTutor, FechaFinTutor,
              FechaInicioDocente, FechaFinDocente,
              TutorActivo, DocenteActivo, GETDATE() AS FechaActual
       FROM eval.Encuesta WHERE IdTipoEncuesta = 1 AND Activa = 1`
    )
    ...
    if (encuesta) {
      const tutorData = await executeQuerySingle(
        `SELECT DISTINCT
         ...
         WHERE ca.NumControl = ? AND g.IdPerio = ? AND ca.Desertor = 0
      ...
      if (tutorData) {
        ...
        tutor = {
          ...
          estadoEvaluacion: evalEstado?.Estado || null
        }
      }
    }
    ...
    if (encuesta) {
      ...
      for (let docente of docentes) {
        ...
        docente.estadoEvaluacion = evalEstado?.Estado || null
      }
    }
  ...
  try {
    const categorias = await executeQuery(
      `SELECT IdCategoria AS id, Nombre, Orden, Activa
       FROM eval.Categoria WHERE Activa = 1 ORDER BY Orden`
    )
    res.json({ categorias })
  } catch (err) {
    res.status(500).json({ error: "Error interno." })
  ...
  try {
    const rubrica = await executeQuery(
      `SELECT IdCategoria AS idCategoria, Valor, Descripcion
       FROM eval.Rubrica ORDER BY IdCategoria, Valor`
    )
    res.json({ rubrica })
  } catch (err) {
    res.status(500).json({ error: "Error interno." })
...
app.get("/api/encuesta/escala", authMiddleware, async (req, res) => {
  res.json({ escala: { 5: "Excelente", 4: "Muy bueno", 3: "Bueno", 2: "Regular", 1: "Deficiente" } })
})
...
app.post("/api/evaluacion/iniciar", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const insertResult = await executeQuery(
      `INSERT INTO eval.EvaluacionDocente
         (IdEncuesta, NumControl, IdDoce, IdGrupo, IdPerio, Estado, IpOrigen)
...
app.post("/api/evaluacion/responder", authMiddleware, async (req, res) => {
  ...
  try {
    const ev = await executeQuerySingle(
    ...
    const totalRequeridas = await executeQuerySingle(
      `SELECT COUNT(*) AS total FROM eval.Pregunta WHERE IdEncuesta = ? AND Requerida = 1 AND Activa = 1`,
      [ev.IdEncuesta]
    )
  ...
  try {
    ...
    await executeQuery(
      `IF EXISTS (SELECT 1 FROM eval.ComentarioEvaluacion WHERE IdEvaluacion = ?)
         ...
         WHERE IdEvaluacion = ?
       ELSE
         INSERT INTO eval.ComentarioEvaluacion
           (IdEvaluacion, NumControl, IdDoce, Comentario, FechaCreacion, Visible)
         VALUES (?, ?, ?, ?, GETDATE(), 1)`,
...
app.get("/api/dashboard/periodos", authMiddleware, soloAdmin, async (req, res) => {
  try {
    const rows = await executeQuery(
      `SELECT IdPerio AS id, Nombre, NombreCorto, Situación AS activo, Inicio, Fin,
              CASE
...
app.post("/api/dashboard/periodo-activo", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    ...
    const nuevo = await executeQuerySingle(
      `SELECT IdPerio AS id, Nombre, NombreCorto, Inicio, Fin FROM dbo.PeriodoEscolar WHERE IdPerio = ?`,
      [periodoId]
    )
...
app.get(
  "/api/dashboard/docentes/:idDocente/periodos/:idPeriodo/grupos",
    ...
    try {
      const rows = await executeQuery(
        `SELECT g.IdGrupo AS id, g.Clave, m.Nombre AS materia
         FROM dbo.Grupo g LEFT JOIN dbo.Materia m ON m.IdMate = g.IdMate
         WHERE g.IdDoce = ? AND g.IdPerio = ? ORDER BY g.Clave`,
...
app.get("/api/dashboard/docente/:idDocente/alumnos", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    const alumnos = await executeQuery(
      `SELECT
         a.NumControl AS numControl, a.NombreCompleto AS nombre,
         a.CorreoE AS correo, a.Semestre AS semestreAlumno,
         g.IdGrupo AS idGrupo, g.Clave AS grupo,
         m.Nombre AS materia, m.NombreCorto AS materiaCorto,
         ca.CalFinal AS calificacion, ca.FaltasTotales AS faltas,
         CASE
       ...
       LEFT JOIN eval.EvaluacionDocente ed ON
         ed.NumControl = ca.NumControl AND ed.IdDoce = g.IdDoce
         AND ed.IdGrupo = g.IdGrupo AND ed.IdPerio = g.IdPerio
       WHERE g.IdDoce = ? AND g.IdPerio = ? AND ca.Desertor = 0
...
app.get("/api/dashboard/departamentos", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    const departamentos = await executeQuery(
      `SELECT DISTINCT dep.IdDepa AS id, dep.Nombre AS nombre
    ...
    if (!departamentos.length) return res.json({ departamentos: [] })

    const resultado = []

    for (const depto of departamentos) {
      const docentesDepto = await executeQuery(
        `SELECT d.IdDoce AS id, d.Grado + ' ' + d.Nombre + ' ' + d.Apellidos AS nombre
         FROM dbo.Docente d
         INNER JOIN dbo.Grupo g ON g.IdDoce = d.IdDoce AND g.IdPerio = ?
      ...
      if (!docentesDepto.length) continue

      const docenteIds = docentesDepto.map(d => d.id)
      const placeholders = docenteIds.map(() => "?").join(",")

      const alumnosCounts = await executeQuerySingle(
        `SELECT COUNT(DISTINCT i.NumControl) AS total_alumnos,
         ...
         JOIN dbo.Grupo g ON g.IdGrupo = i.IdGrupo
           AND g.IdDoce IN (${placeholders}) AND g.IdPerio = ?
         LEFT JOIN eval.EvaluacionDocente e ON e.NumControl = i.NumControl
           AND e.IdDoce = g.IdDoce AND e.IdPerio = ? AND e.Estado = 3
         WHERE i.Activa = 1`,
      ...
      const evCounts = await executeQuerySingle(
        `SELECT COUNT(DISTINCT IdDoce) AS evaluados
         FROM eval.EvaluacionDocente
         WHERE IdDoce IN (${placeholders}) AND IdPerio = ? AND Estado = 3`,
        [...docenteIds, periodoId]
      )

      const promediosDocentes = await executeQuery(
        `SELECT e.IdDoce, ROUND(AVG(CAST(r.Calificacion AS FLOAT)), 2) AS promedio
         FROM eval.EvaluacionDocente e
         JOIN eval.RespuestaEvaluacion r ON r.IdEvaluacion = e.IdEvaluacion
         WHERE e.IdDoce IN (${placeholders}) AND e.IdPerio = ? AND e.Estado = 3
         GROUP BY e.IdDoce`,
        [...docenteIds, periodoId]
      )
      ...
      for (const doc of docentesDepto) {
        ...
        const docAlumnos = await executeQuerySingle(
          `SELECT COUNT(DISTINCT i.NumControl) AS total,
        ...
        docentesConStats.push({
          ...
          nombre: doc.nombre,
          promedio,
          clasificacion: promedio >= 4.5 ? "EXCELENTE"
      ...
      resultado.push({
        id: depto.id,
        nombre: depto.nombre,
        totalDocentes: docentesDepto.length,
        docentesEvaluados: evCounts?.evaluados ?? 0,
        promedioGeneral,
...
app.get("/api/dashboard/resultados", authMiddleware, soloAdmin, async (req, res) => {
  ...
  try {
    ...
    let pCatQuery = `SELECT c.IdCategoria, c.Nombre,
       ...
       LEFT JOIN eval.EvaluacionDocente e ON e.IdEvaluacion = r.IdEvaluacion
         AND e.IdDoce = ? AND e.IdPerio = ? AND e.Estado = 3`
    const pCatParams = [docenteId, periodoId]
    if (grupoId) { pCatQuery += ` AND e.IdGrupo = ?`; pCatParams.push(grupoId) }
    pCatQuery += ` GROUP BY c.IdCategoria, c.Nombre ORDER BY c.IdCategoria`
    const promediosCat = await executeQuery(pCatQuery, pCatParams)

    let cQuery = `SELECT COUNT(DISTINCT i.NumControl) AS total_alumnos,
       ...
       LEFT JOIN eval.EvaluacionDocente e ON e.NumControl = i.NumControl
         AND e.IdDoce = ? AND e.IdPerio = ? AND e.Estado = 3
       WHERE i.Activa = 1`
    const cParams = [docenteId, periodoId, docenteId, periodoId]
    if (grupoId) { cQuery += ` AND g.IdGrupo = ?`; cParams.push(grupoId) }
    const counts = await executeQuerySingle(cQuery, cParams)

    let compQuery = `SELECT a.NumControl AS numControl, a.NombreCompleto AS nombre,
       ...
       WHERE e.IdDoce = ? AND e.IdPerio = ? AND e.Estado = 3`
    const compParams = [docenteId, periodoId]
    if (grupoId) { compQuery += ` AND e.IdGrupo = ?`; compParams.push(grupoId) }
    const completaron = await executeQuery(compQuery, compParams)

    let fQuery = `SELECT a.NumControl AS numControl, a.NombreCompleto AS nombre,
       c.NombreCorto AS carrera, g.Clave AS grupo
       FROM eval.Inscripcion i
       JOIN dbo.Grupo g ON g.IdGrupo = i.IdGrupo AND g.IdDoce = ? AND g.IdPerio = ?
    ...
    const fParams = [docenteId, periodoId, docenteId, periodoId]
    if (grupoId) { fQuery += ` AND g.IdGrupo = ?`; fParams.push(grupoId) }
    const faltantes = await executeQuery(fQuery, fParams)

    const promediosMap = {}
    promediosCat.forEach(p => { promediosMap[p.IdCategoria] = Number(p.promedio) })
    const valores = promediosCat.map(p => Number(p.promedio)).filter(v => v > 0)
    const promedioGeneral = valores.length
    ...
    res.json({
      ...
      promedioGeneral,
      clasificacion: promedioGeneral >= 4.5 ? "EXCELENTE"
        : promedioGeneral >= 3.5 ? "MUY BUENO"
        : promedioGeneral >= 2.5 ? "BUENO"
        : promedioGeneral >= 1.5 ? "REGULAR"
        : promedioGeneral > 0 ? "DEFICIENTE"
...
app.get(
  "/api/dashboard/docentes/:idDocente/comentarios",
  authMiddleware, soloAdmin,
  async (req, res) => {
    const { idDocente } = req.params
    const { idPeriodo } = req.query
    ...
    try {
      const comentarios = await executeQuery(
        `SELECT ce.IdComentario, ce.Comentario, ce.FechaCreacion,
                a.NombreCompleto AS alumno
         FROM eval.ComentarioEvaluacion ce
         INNER JOIN eval.EvaluacionDocente ed ON ed.IdEvaluacion = ce.IdEvaluacion
         INNER JOIN dbo.Alumno a ON a.NumControl = ce.NumControl
         WHERE ce.IdDoce = ? AND ed.IdPerio = ? AND ce.Visible = 1
           AND ce.Comentario IS NOT NULL AND LTRIM(RTRIM(ce.Comentario)) != ''
