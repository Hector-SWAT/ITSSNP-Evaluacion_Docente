...
function exportarCSV(resultado, grupoLabel) {
  ...
  const rows = [
    ...
    ...completaron.map(a=>[a.numControl,a.nombre,a.grupo||"—",a.carrera]),
    [], ["PENDIENTES"], ["No.Control","Nombre","Grupo","Carrera"],
    ...faltantes.map(a=>[a.numControl,a.nombre,a.grupo||"—",a.carrera]),
  ]
...
function TarjetasDepartamentos({ idPeriodo, onVerDocente }) {
  ...
  const [expanded, setExpanded] = useState(null)  // id_depa del ranking abierto
  ...
  return (
    <div className="db-depto-wrap">
      ...
      {/* Tarjetas horizontales scrollables */}
...
export default function Dashboard() {
  ...
  const grupoLabel = idGrupo ? grupos.find(g => String(g.id) === String(idGrupo))?.clave : null
  ...
  useEffect(() => {
    ...
    getGruposAPI(Number(idDocente), Number(idPeriodo))
      .then(g => { setGrupos(g); setErrorGrupos(null); setIdGrupo("") })
    ...
    <>
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* Tarjetas de departamentos — scroll horizontal */
      ...
      <div className="db-root">
        ...
        <div className="db-body">
          ...
          {/* ══ TARJETAS DE DEPARTAMENTOS (siempre encima) ══ */}
            ...
            <div className="db-fgroup">
              ...
              <select className="db-fsel" value={idPeriodo} onChange={e => setIdPeriodo(e.target.value)} disabled={cargandoPeriodos || !!errorPeriodos}>
                {cargandoPeriodos ? <option>Cargando…</option> : errorPeriodos ? <option>Error</option> : <>
                  ...
                  {periodos.map(p => <option key={p.id} value={p.id}>{p.nombre}{(p.activo===1||p.activo===true)?" ★ Activo":""}</option>)}
                </>}
              </select>
            </div>
            ...
            <div className="db-fgroup">
              ...
              <select className="db-fsel" value={idGrupo} onChange={e => setIdGrupo(e.target.value)} disabled={!idDocente||!idPeriodo||cargandoGrupos||!!errorGrupos}>
                {!idDocente||!idPeriodo ? <option value="">Selecciona docente y periodo</option>
                  ...
                  : <><option value="">— Todos los grupos —</option>{grupos.map(g=><option key={g.id} value={g.id}>{g.clave}</option>)}</>}
              </select>
            </div>
          </div>
                ...
                <div className="db-card">
                  ...
                  <div className="db-table-wrap">
                    <table className="db-table">
                      <thead><tr><th>No. Control</th><th>Nombre</th><th>Grupo</th><th>Carrera</th><th>Estado</th></tr></thead>
                      <tbody>
              ...
              <div className="db-card">
                ...
                <div style={{ overflowX:"auto" }}>
                  <table className="db-table" style={{ minWidth:600 }}>
                    <thead><tr><th style={{ width:30 }}>#</th><th>Criterio</th><th style={{ width:100, textAlign:"center" }}>Promedio</th><th style={{ width:130 }}>Clasificación</th><th style={{ width:160 }}>Nivel</th></tr></thead>
                    <tbody>
                      {CATEGORIAS.map((cat, i) => {
